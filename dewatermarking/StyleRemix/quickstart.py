import os
import sys
directory = '../WaterMarking'
sys.path.append(directory)
os.environ['TRANSFORMERS_CACHE'] = '../cache/'
os.environ['HF_HOME'] = '../cache/'
os.environ['HUGGINGFACE_HUB_CACHE'] = '../cache/'
from watermark.extended_watermark_processor import WatermarkLogitsProcessor, WatermarkDetector
import torch
...
login(token = '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')
def main(args):
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_id, add_bos_token=True, add_eos_token=False, padding_side="left",use_auth_token=True)
    tokenizer.add_special_tokens({'pad_token': '<padding_token>'})
    base_model = AutoModelForCausalLM.from_pretrained(model_id,use_auth_token=True,device_map="auto") # device_map="auto" requires accelerate
    ...
    model = PeftModel.from_pretrained(base_model, MODEL_PATHS[FIRST_MODEL], adapter_name=FIRST_MODEL,device_map="auto")
